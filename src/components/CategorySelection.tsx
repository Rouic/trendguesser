          ...
          <button
            onClick={() => {
              ...
              if (typeof window !== 'undefined') {
                ...
                if (currentGameId && !sessionStorage.getItem(`game_${currentGameId}`)) {
          ...
          <button
            onClick={() => {
              if (typeof window !== 'undefined') {
                ...
                for (let i = 0; i < sessionStorage.length; i++) {
                  const key = sessionStorage.key(i);
                  if (key && (key.startsWith('game_') || key === 'current_game_id' || key === 'mock_user_uid')) {
                    gameKeys.push(key);
                  }
                }
                ...
                console.log(`Cleared ${gameKeys.length} items from session storage:`, gameKeys);
                
                // Optionally, reload the page to reset all state
                if (confirm('Session storage cleared. Reload page to reset all application state?')) {
