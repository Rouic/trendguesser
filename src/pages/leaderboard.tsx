...
const LeaderboardPage = () => {
  const [category, setCategory] = useState<SearchCategory>("snacks");
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-between px-4 sm:px-8 py-4 sm:py-8 relative overflow-hidden">
      ...
      <div className="max-w-4xl w-full mx-auto text-center relative z-10 flex flex-col items-center py-8 sm:py-12">
        <h1 className="text-3xl sm:text-4xl font-display text-game-neon-yellow tracking-wider mb-6 animate-glow font-display-fallback">
          TREND GUESSER LEADERBOARD
        </h1>
        ...
        <div className="w-full bg-black/40 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/10">
          {loading ? (
            <div className="p-8 text-center">
              ...
              <p className="text-white/70 font-game-fallback">
                Loading leaderboard...
              </p>
            </div>
          ) : error ? (
            ...
            <div className="p-8 text-center">
              <p className="text-white/70 font-game-fallback">
                No scores for this category yet. Be the first!
              </p>
              <Link
                href="/game"
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="px-4 py-3 text-left text-white/70 font-game-fallback">
                    Rank
                  </th>
                  <th className="px-4 py-3 text-left text-white/70 font-game-fallback">
                    Player
                  </th>
                  <th className="px-4 py-3 text-right text-white/70 font-game-fallback">
                    Score
                  </th>
                </tr>
              </thead>
                ...
                {leaderboard.map((player, index) => (
                  <motion.tr
                    ...
                    className={`border-b border-white/10 hover:bg-white/5 ${
                      index === 0
                        ? "bg-game-neon-gold/10"
                        : index === 1
                        ? "bg-game-neon-silver/10"
                        : index === 2
                        ? "bg-game-neon-bronze/10"
                        : ""
                    }`}
                  >
                    <td className="px-4 py-3 text-left">
                      <span
                        ...
                        ${
                          index === 0
                            ? "bg-game-neon-gold/20 text-game-neon-gold"
                            : index === 1
                            ? "bg-game-neon-silver/20 text-game-neon-silver"
                            : index === 2
                            ? "bg-game-neon-bronze/20 text-game-neon-bronze"
        ...
        <Link
          href="/game"
          className="mt-8 px-8 py-3 bg-black/40 backdrop-blur-sm rounded-xl border-2 border-game-neon-yellow/50 text-game-neon-yellow font-bold font-game-fallback text-xl hover:bg-black/60 hover:scale-105 transition-all duration-300 shadow-neon-yellow-sm"
        >
          PLAY NOW
        </Link>
      </div>
