...
export async function getPlayer(playerId: string): Promise<PlayerData | null> {
  try {
    ...
    // Handle different response formats safely - neon might return array or {rows:[]} format
    let rows = [];
...
export async function updatePlayer(playerId: string, updates: Partial<PlayerData>): Promise<void> {
  try {
    ...
    if (playerExists) {
      ...
      console.log(`Updated player ${playerId} with fields: ${fields.join(', ')}`);
    } else {
      // Create new player
      await sql(
      ...
      console.log(`Created new player ${playerId} with name: ${updates.name || 'Player'}`);
    }
  } catch (error) {
...
export async function updateHighScore(playerId: string, category: SearchCategory, score: number, playerName?: string): Promise<void> {
  try {
    if (!DATABASE_URL) {
    ...
    if (player) {
      ...
      if (!highScores[category] || score > highScores[category]) {
        ...
        // If playerName is provided, update it as well
        const playerUpdate: Partial<PlayerData> = { highScores };
        if (playerName && playerName.trim() !== '') {
          playerUpdate.name = playerName;
        }
        
        await updatePlayer(playerId, playerUpdate);
        
        // Update the leaderboard - use player's name from the record or the provided name
        const nameToUse = playerName && playerName.trim() !== '' 
          ? playerName 
          : (player.name || 'Player');
          
        await updateLeaderboard(playerId, category, score, nameToUse);
      }
    } else {
      // Create a new player with this high score
      highScores = { [category]: score };
      
      // Use provided name or default to "Player"
      const nameToUse = playerName && playerName.trim() !== '' ? playerName : 'Player';
...
export async function updateLeaderboard(
  ...
  try {
    ...
    if (Array.isArray(existingEntry)) {
      rows = existingEntry;
    } else if (existingEntry && typeof existingEntry === 'object' && 'rows' in existingEntry && Array.isArray(existingEntry.rows)) {
      rows = existingEntry.rows;
    }
    
    if (rows.length > 0) {
      const currentScore = parseInt(rows[0].score || '0');
