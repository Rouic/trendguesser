...
async function scrapeSearchVolume(term) {
    try {
        ...
        const exploreResponse = await axios_1.default.get(exploreUrl, exploreOptions);
        ...
        const interestOverTimeWidget = exploreData.widgets.find((widget) => widget.id === 'TIMESERIES');
        ...
        const timelineResponse = await axios_1.default.get(timelineUrl, timelineOptions);
        ...
        const timelinePoints = timelineData.default.timelineData;
        let totalInterest = 0;
        ...
        if (relatedQueriesWidget) {
            try {
                ...
                const queriesResponse = await axios_1.default.get(queriesUrl, queriesOptions);
                ...
                if (queriesData && queriesData.default && queriesData.default.rankedList) {
                    ...
                    const risingList = queriesData.default.rankedList.find((list) => list.title === 'Rising');
                    ...
                    if (relatedQueries.length === 0) {
                        const topList = queriesData.default.rankedList.find((list) => list.title === 'Top');
...
async function scrapeWikipediaPageViews(term) {
    try {
        ...
        const pageViewsResponse = await axios_1.default.get(pageViewsUrl, {
            timeout: 5000
        });
...
async function scrapeTrendingTerms(category) {
    try {
        ...
        const response = await axios_1.default.get(trendsUrl, {
            params,
            timeout: 10000,
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36'
...
async function getMockTrendingTerms(category) {
